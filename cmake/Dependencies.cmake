...
function(_gr4_incubator_find_httplib out_target)
  set(_httplib_extra_libs "")
  find_package(OpenSSL QUIET)
  if(TARGET OpenSSL::SSL)
    list(APPEND _httplib_extra_libs OpenSSL::SSL)
  endif()
  find_package(ZLIB QUIET)
  if(TARGET ZLIB::ZLIB)
    list(APPEND _httplib_extra_libs ZLIB::ZLIB)
  endif()
  foreach(_brotli_module IN ITEMS libbrotlicommon libbrotlidec libbrotlienc)
    _gr4_incubator_optional_pkgconfig(_brotli_target _brotli_found "${_brotli_module}")
    if(_brotli_found)
      list(APPEND _httplib_extra_libs "${_brotli_target}")
    endif()
  endforeach()

  find_package(httplib CONFIG QUIET)
  if(TARGET httplib::httplib)
    target_link_libraries(httplib::httplib INTERFACE ${_httplib_extra_libs})
  ...
  if(_httplib_found)
    target_link_libraries(${_httplib_pkg} INTERFACE ${_httplib_extra_libs})
  ...
  if(_httplib_found_l)
    target_link_libraries(${_httplib_pkg_l} INTERFACE ${_httplib_extra_libs})
...
function(gr4_incubator_resolve_dependencies)
  ...
  if(ENABLE_PLUGINS)
    ...
    if(NOT TARGET gr4_incubator_gnuradio4_plugin)
      if(TARGET gnuradio4::gnuradio-plugin)
        ...
        find_library(GR4I_GNURADIO4_PLUGIN_LIB NAMES gnuradio-plugin REQUIRED)
        add_library(gr4_incubator_gnuradio4_plugin UNKNOWN IMPORTED)
        set_target_properties(gr4_incubator_gnuradio4_plugin PROPERTIES
          IMPORTED_LOCATION "${GR4I_GNURADIO4_PLUGIN_LIB}")
        target_link_libraries(gr4_incubator_gnuradio4_plugin INTERFACE "${GR4I_GNURADIO4_TARGET}")
        set(_gr4i_plugin_target gr4_incubator_gnuradio4_plugin)
      endif()
    else()
      set(_gr4i_plugin_target gr4_incubator_gnuradio4_plugin)
    endif()
  ...
  if(ENABLE_EXAMPLES OR ENABLE_TESTING OR ENABLE_PLUGINS)
  ...
  if(ENABLE_EXAMPLES OR ENABLE_PLUGINS)
